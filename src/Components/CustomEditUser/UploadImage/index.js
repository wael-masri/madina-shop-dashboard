...
const UploadImage = ({ setImTyping, setImageUp, setOpenCrop,setImgAfterCrop,dataUser,setUrlImage,urlImage }) => {

  useEffect(() => {
  const editSomeData = () => {
   const urlImageFromDB = dataUser.profileImage;
  ...
  return !urlImage ? (
    ...
    <BoxUpload>
      ...
      <ButtonSelectFile onClick={() => {setImageUp(null);setImgAfterCrop(null);setUrlImage(null);setImTyping(true);}}>
        Cancel
      </ButtonSelectFile>
    </BoxUpload>
  );
