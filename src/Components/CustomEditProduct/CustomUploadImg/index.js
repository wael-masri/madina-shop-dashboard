...
const CustomUploadImg = ({ setInputData, errImage, oldData,setImTyping }) => {
  ...
  useEffect(() => {
    setInputData((prevStat) => ({
      ...prevStat,
      imageCover: otherState.imageAfterCrop,
    }));
  }, [otherState.imageAfterCrop]);
  useEffect(() => {
    const editSomeData = () => {
      let UrlFormDB = oldData.imageCover;
      dispatch(setUrlImageAfterUpload(UrlFormDB));
      if (otherState.imageAfterCrop === null) {
        setInputData((prevStat) => ({ ...prevStat, imageCover: UrlFormDB }));
      }
    };
    editSomeData();
  }, []);
    ...
    <BoxUpload>
      ...
      <ButtonSelectFile
        onClick={() => {
          ...
          setInputData((prevStat) => ({ ...prevStat, imageCover: null }));
          setImTyping(true);
