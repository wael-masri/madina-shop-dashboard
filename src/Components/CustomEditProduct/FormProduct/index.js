...
const FormProduct = ({
  ...
  useEffect(() => {
    ...
    const editSomeData = () => {
      setInputData((prevStat) => ({ ...prevStat, status: oldData.status }));
      let objval = { value: oldData.category._id };
      handleChangeCategory(objval);
    };
    getCategories();
    editSomeData();
    getBrand();
            ...
            <Controller
              ...
              render={({ field: { onChange, value, ref } }) => (
                <SelectReact
                  ...
                  value={subCategories.filter((c) => { 
                    
                  return  value.find(val => c.value === val);
