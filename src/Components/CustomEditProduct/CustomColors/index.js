...
const CustomColors = ({ setInputData, oldData,setImTyping }) => {
  ...
  useEffect(() => {
    const editSomeElement = () => {
      let mergArr = {};
       mergArr = colorsImpossible.concat(oldData);
      mergArr = mergArr.filter((item, index) => {
        return mergArr.indexOf(item) == index;
      });
      setColorsImpossible(mergArr);
      setSelectColors(oldData);
    };
    editSomeElement();
  }, []);
